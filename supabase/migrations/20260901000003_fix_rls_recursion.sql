...
DROP POLICY IF EXISTS "users: admin manage own institute" ON users;
DROP POLICY IF EXISTS "users: platform_admin full access" ON users;
DROP POLICY IF EXISTS "users: insert own on signup" ON users;
DROP POLICY IF EXISTS "users_select_own_or_tenant" ON users;
DROP POLICY IF EXISTS "users_update_own_profile" ON users;
DROP POLICY IF EXISTS "users_admin_manage_own_institute" ON users;
DROP POLICY IF EXISTS "users_platform_admin_full_access" ON users;
DROP POLICY IF EXISTS "users_insert_own_on_signup" ON users;

DROP POLICY IF EXISTS "institutes: platform_admin full access" ON institutes;
DROP POLICY IF EXISTS "institutes_platform_admin_full_access" ON institutes;

-- 3. Re-create non-recursive policies using SECURITY DEFINER functions

CREATE POLICY "users_select_own_or_tenant" ON users
...
CREATE POLICY "users_admin_manage_own_institute" ON users
...
CREATE POLICY "users_platform_admin_full_access" ON users
  FOR ALL TO authenticated
...
CREATE POLICY "institutes_platform_admin_full_access" ON institutes
  FOR ALL TO authenticated
  USING (public.current_user_role() = 'platform_admin');
